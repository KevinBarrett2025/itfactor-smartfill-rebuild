...
- The first rebuild slice is locally gated:
  - legacy audit
...
| --- | --- | --- | --- | --- |
| 1 | SF-REBUILD-003 | Launch SmartFill from project/session/take review into `SmartFillSessionContext` | OPEN | First flagship shell integration step |
| 2 | SF-REBUILD-004 | Replace legacy SmartFill settings/editor surfaces with one bounded workspace | OPEN | Must preserve shipped shell fit from SelfTapeStudio |
...
- No commit should claim promoted truth until this repo has a configured remote and authority branch.

---

## Next Action
1. Commit and push the bootstrap rebuild slice on `gm/smartfill-itfactor-rebuild`.
2. Integrate SmartFill launch from project/session/take review.
3. Replace legacy settings/editor UI with one bounded SmartFill workspace.
