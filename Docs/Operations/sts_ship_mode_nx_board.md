...
## Next Action
1. Use `SF-REBUILD-047` as the next safe workspace baseline for a device smoke before choosing the next intentional compare/playback slice. The current shell now keeps live playback, momentary compare, precise frame-step nudging, direct canvas precision scrubbing, a true shared-playhead compare viewer, a temporary split-wipe gesture, one compact explicit compare-mode toolbar, one grouped preview-side compare control, a pinned-preview inline wipe mode, and one shared compare-selection seam in place while the HomeScreen portrait-player SmartFill chip still opens the rebuild workspace correctly.
