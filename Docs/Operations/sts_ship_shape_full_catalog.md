...
## Next Action
1. Choose the next intentional flagship SmartFill workspace phase now that `SF-REBUILD-028` has proven the tray can stay light while deeper controls live in dedicated sheets.
2. Implement that next slice on GM with Gate A plus focused SmartFill parity before any promotion decision.
3. Keep standalone derivation synchronized in every phase so the utility app inherits the same fixed preview + tray/rail chrome plus the secondary-sheet split for denser tools.
