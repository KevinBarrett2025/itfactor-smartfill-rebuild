...
## Next Action
1. Choose the next intentional flagship SmartFill workspace phase now that `SF-REBUILD-029` has clarified which SmartFill controls deserve true inline ownership versus sheet-only drill-ins.
2. Implement that next slice on GM with Gate A plus focused SmartFill parity before any promotion decision.
3. Keep standalone derivation synchronized in every phase so the utility app inherits the same fixed preview + tray/rail chrome, inline ownership model, and secondary-sheet split for denser tools.
