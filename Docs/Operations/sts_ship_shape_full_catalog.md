...
| SF-REBUILD-004 | Bounded SmartFill workspace replacement inside itFactor shell | OPEN | Initial `SmartFillWorkspaceView` landed through the review/player launch path; repository-backed return is now live, but broader duplicate settings/preview surface cutover is still pending |
...
## Next Action
1. Commit and push the repository-adoption slice.
2. Continue replacing duplicate legacy settings/editor surfaces with the bounded workspace until it fully owns SmartFill entry and return.
3. Start deleting `DELETE_AFTER_CUTOVER` SmartFill seams after the rebuild path becomes the sole active path.
