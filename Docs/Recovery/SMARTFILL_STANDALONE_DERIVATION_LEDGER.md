...
## Current Next Step
Use the `SF-REBUILD-050` baseline for the next safe device smoke. After that, decide the next intentional shared-workspace evolution once duplicate screens are gone, one rebuild-owned defaults entry exists, the workspace shows the real SmartFill preview, the main product decisions are exposed as explicit user-facing lanes, the save/return story reflects the real launch context, the common treatment controls live directly in the workspace, live save progress plus explicit stay-vs-return control are anchored, dirty-after-save truth stays honest, shipped-style quick fill presets are back, save-outcome messaging explains what changes before save, treatment-finish presets plus an explicit pre-save stay/return choice now exist inside the main workspace, save/update affordances now fully match the chosen finish behavior, completed-state guidance now names the actual saved SmartFill take, both the post-save primary action and the automatic return path now reopen that same saved result through a shared follow-up route, the reopened destinations themselves now explicitly identify that result as the saved SmartFill output, the workspace chrome now scales through a fixed preview plus contextual tray/rail shell, the active tray now uses compact quick groups with one-at-a-time fine tuning, denser controls now route through explicit secondary sheets, inline ownership is now explicit for the highest-frequency editing decisions, the preview now carries the active tool’s current values plus one relevant drill-in action, the pinned preview now supports live play/pause and scrub transport, that live transport now also stays embedded directly inside the preview canvas instead of sitting in a disconnected slab, the workspace now exposes an original-source compare sheet directly beside that live preview, the main preview now switches inline between result and original states with the same stronger transport language in both places, that source/result compare now preserves shared playback state instead of resetting on every toggle, the preview now also supports momentary hold-to-compare directly on the canvas, the preview transport now also supports precise frame-step nudging, the preview canvas now also supports bounded horizontal precision scrubbing with a temporary HUD, the larger compare sheet now behaves like a true source-versus-current viewer at the same shared playhead, the larger compare sheet now also supports a temporary split-wipe gesture with one ephemeral divider, the compare sheet now also uses one compact explicit `Source` / `Current` / `Wipe` toolbar, the compare sheet now also remembers the last compare mode and pinned wipe position within the active workspace session, the preview focus deck now also exposes one grouped compare control that mirrors that remembered compare state, the pinned preview now also supports direct `Source`, `Current`, and pinned `Wipe` modes, the pinned preview and larger compare viewer now also share one compare-selection seam directly, the HomeScreen portrait-player SmartFill chip now enters the same rebuild workspace seam instead of dying at the player boundary, the shared preview wrapper now preserves the original unified `AVPlayer` instead of crashing by attaching its `AVPlayerItem` to a second player, the workspace now keeps one authoritative top-toolbar action seam instead of duplicating save/close actions in a second bottom bar, the workspace now uses the same cinematic studio shell that later fits the standalone and macOS SmartFill surfaces, the preview now preserves visible portrait media at rest through a poster-frame seam, and the advanced adjustments surface now already behaves like a compact studio drill-in instead of a pop-brand legacy list. The standalone utility can now derive from the rebuild workspace plus hidden static-session shell without carrying dormant settings/dashboard wrappers, a separate preview shell, utility-only product copy, fake return-target assumptions, a separate treatment-tuning shell, a utility-only save-progress layer, a fake completed state after unsaved changes, a utility-only explanation layer for save outcomes, a separate finish-state chooser, generic fake result naming, a utility-only auto-close path, a generic post-save `Return` action that hides the real saved result identity, a silent reopen destination that gives no saved-result context, a long utility-only settings page that collapses as new SmartFill tools are added, a placeholder SmartFill chip that resolves an intent but has no live launch seam, a compare flow that loses scrub/play context whenever users switch between original and result, a redundant second action bar, a pop-marketing shell that conflicts with the flagship studio editor, or a preview wrapper that silently reuses one `AVPlayerItem` across two players. The next shared candidate after this preview-visibility and studio-ownership slice lands is tightening the remaining stacked-card feel and promoting only the highest-value background/subject controls inline without regressing back into verbose settings chrome.
