...
## Current Next Step
Decide the next intentional shared-workspace evolution after duplicate screens are gone, one rebuild-owned defaults entry exists, the workspace shows the real SmartFill preview, the main product decisions are exposed as explicit user-facing lanes, the save/return story reflects the real launch context, the common treatment controls live directly in the workspace, live save progress plus explicit stay-vs-return control are anchored, and dirty-after-save truth now stays honest. The standalone utility can now derive from the rebuild workspace plus hidden static-session shell without carrying dormant settings/dashboard wrappers, a separate preview shell, utility-only product copy, fake return-target assumptions, a separate treatment-tuning shell, a utility-only save-progress layer, or a fake completed state after unsaved changes.
