...
- `STSiPhone/STSiPhone/Features/SmartFill/Rebuild/SmartFillWorkspacePreviewTransportOwnership` / `SmartFillWorkspacePreviewPlaybackState.syncingObservedPlayback(...)` / shared preview transport-ownership seam that preserves parent play intent while the active preview owner publishes observed playback plus time (declared in `SmartFillWorkspaceView.swift`)
- `STSiPhone/STSiPhone/Core/VideoPipeline/SmartFill/SmartFillCIBuilder.resolvedForegroundScale(...)` / real foreground zoom composition seam that now applies workspace framing/zoom to the rendered SmartFill output instead of leaving it as tray-only UI
...
48. When the pinned preview exposes a visible play affordance during `Wipe` compare, one preview layer must remain the sole playback and hit-testing owner. Never leave both preview layers non-interactive in pinned wipe mode, or the editor will render a visible but dead transport.
